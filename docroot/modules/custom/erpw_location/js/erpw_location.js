  ...
  Drupal.behaviors.location = {
    attach(context, settings) {
      ...
      $("select.location_options").change(function (event) {
        ...
        const zeroTid = event.target.value;
          ...
          if (zeroKey === zeroTid) {
            for (const oneKey in zeroValue) {
              ...
              if (oneKey === "children") {
                ...
                const select = $("#location-level-1 .level_1")[0];
                const level_1 = Drupal.t(`Select ${zeroValue.level_label}`);
                document.querySelector("#location-level-1 label").textContent =
                  level_1;
                $("#location-level-1 .level_1").empty();
      ...
      $("#location-level-1 .level_1").change(function (event) {
        event.preventDefault();
        const zeroTid = $("select.location_options").val();
        const oneTid = $("#location-level-1 .level_1").val();
        for (const zeroKey in termsArray) {
          const zeroValue = termsArray[zeroKey];
          // checks if the parent id is equal to level zero terms.
          if (zeroKey === zeroTid) {
            const newoptions = termsArray[zeroKey].children[oneTid].children;
            $("#location-level-2").css({
              display: "block",
            });
            const select = $("#location-level-2 .level_2")[0];
            $("#location-level-2 .level_2").empty();
            const level_2 = Drupal.t(
              `Select ${termsArray[zeroKey].children[oneTid].level_label}`,
            );
            document.querySelector("#location-level-2 label").textContent =
              level_2;
      ...
      $("#location-level-2 .level_2").change(function (event) {
        ...
        const zeroTid = $("select.location_options").val();
        const oneTid = $("#location-level-1 .level_1").val();
        const twoTid = $("#location-level-2 .level_2").val();
        for (const zeroKey in termsArray) {
          const zeroValue = termsArray[zeroKey];
          // checks if the parent id is equal to level zero terms.
          if (zeroKey === zeroTid) {
            const newoptions =
              termsArray[zeroKey].children[oneTid].children[twoTid].children;
            ...
            const select = $("#location-level-3 .level_3")[0];
            $("#location-level-3 .level_3").empty();
            const level_3 = Drupal.t(
              `Select ${termsArray[zeroKey].children[oneTid].children[twoTid].level_label}`,
            );
            document.querySelector("#location-level-3 label").textContent =
              level_3;
      ...
      $("#location-level-3 .level_3").change(function (event) {
        ...
        const oneTid = $("#location-level-1 .level_1").val();
        const twoTid = $("#location-level-2 .level_2").val();
        const threeTid = $("#location-level-3 .level_3").val();
        for (const zeroKey in termsArray) {
          const zeroValue = termsArray[zeroKey];
          // checks if the parent id is equal to level zero terms.
          if (zeroKey === zeroTid) {
            ...
            for (const keys in newoptions) {
              if (keys === "children") {
                ...
                const select = $("#location-level-4 .level_4")[0];
                $("#location-level-4 .level_4").empty();
                const level_4 = Drupal.t(
                  `Select ${termsArray[zeroKey].children[oneTid].children[twoTid].children[threeTid].level_label}`,
                );
                document.querySelector("#location-level-4 label").textContent =
                  level_4;
                select.add(new Option(level_4, 0));
                for (const newKey in newoptions[keys]) {
                  select.add(new Option(newoptions[keys][newKey].name, newKey));
                }
              }
            }
          }
