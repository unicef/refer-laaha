...
class NotificationEntityForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Notification entity.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Notification entity.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.notification_entity.canonical', ['notification_entity' => $entity->id()]);
  }
