(function ($, Drupal, drupalSettings) {
  Drupal.behaviors.erpw_webform_location_list = {
    attach: function (context, settings) {
      ...
      // Set label for level.
      var level_1 = Drupal.t("Select " + termsArray[countryID]["level_label"]);
      $("select.level_1").siblings("label").text(level_1);
      $("select.level_1").empty();
      for (const newKey in level1_options) {
        LevelOneselect.add(new Option(level1_options[newKey]["name"], newKey));
      }
      ...
      var level_2 = Drupal.t(
        "Select " + termsArray[countryID]["children"][LevelOneID]["level_label"]
      );
      $("select.level_2").siblings("label").text(level_2);
      $("select.level_2").empty();
      for (const newKey in level2_options) {
        LevelTwoselect.add(new Option(level2_options[newKey]["name"], newKey));
      }
      $("select.level_2").val(LevelTwoID);

      // Options for level3.
      level3_options =
        termsArray[countryID]["children"][LevelOneID]["children"][LevelTwoID][
          "children"
        ];
