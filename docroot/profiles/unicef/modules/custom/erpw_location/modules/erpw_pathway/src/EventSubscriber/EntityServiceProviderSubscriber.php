...
/**
 * Class EntityServiceProviderSubscriber.
 *
 * To alter the node forms.
 */
class EntityServiceProviderSubscriber implements EventSubscriberInterface {
  ...
  public function alterForm(FormAlterEvent $event): void {
    ...
    if ($form_id == 'node_service_provider_edit_form') {
      $form['#title'] = $this->t('Update Service');
  ...
  public static function getTermParents(NodeInterface $node) {
    $referenced_entities = $node->get('field_location_sp')->referencedEntities();
    $parents = [];
    $loc_tid = [];
  ...
  protected function saveLocationField($entity, $location) {
    if (is_array($location)) {
      foreach ($location as $value) {
        $entity->field_location_sp[] = ['target_id' => $value];
      }
    }
    else {
      $entity->field_location_sp->target_id = $location;
    }
    $entity->save();
  }
